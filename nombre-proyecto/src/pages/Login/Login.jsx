...
function Login() {
    ...
    function iniciarSesion() {
        ...
        const indexUsuario = usuariosData.findIndex((itemUsuarios) => {
            return ((itemUsuarios.email === loginEmailValor ||
                itemUsuarios.name === loginEmailValor) && itemUsuarios.password
                === loginPasswordValor)
            }
        )// si son iguales iniciar sesion
        if (indexUsuario >= 0) {
            const user = usuariosData[indexUsuario]
// igual que local pero sesion pedir que guarde la cadena de texto
    ...
    return (
        <div className="form">
            ...
            0-1.442-.725-1.442-1.914z"></path>
                ...
                <input value={loginEmailValor} onChange={(event) => { setloginEmailValor(event.target.value) }} autoComplete="off" placeholder="Name" class="input-field " type="text" />
            </div>
            <div className="field">
                <svg className="input-icon" xmlns="http://www.w3.org/2000/svg"
                    width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
